	...
	if (!ctx.avail_meta_count) {
		...
		if (!ctx.avail_meta_area_count && ctx.brk!=-1) {
			...
			if (brk(new) != new) {
				...
				// printf("alloc_meta 10\n");
				if (need_guard) mmap((void *)ctx.brk, pagesize,
					PROT_NONE, MAP_ANON|MAP_PRIVATE|MAP_FIXED, -1, 0);
				// printf("alloc_meta 11\n");
				ctx.brk = new;
				// printf("alloc_meta 12\n");
				ctx.avail_meta_areas = (void *)(new - pagesize);
				ctx.avail_meta_area_count = pagesize>>12;
				need_unprotect = 0;
			}
		}
		// printf("alloc_meta 13\n");
		if (!ctx.avail_meta_area_count) {
			size_t n = 2UL << ctx.meta_alloc_shift;
			p = mmap(0, n*pagesize, PROT_NONE,
				MAP_PRIVATE|MAP_ANON, -1, 0);
			// printf("alloc_meta 14\n");
			if (p==MAP_FAILED) return 0;
			// printf("alloc_meta 15\n");
			ctx.avail_meta_areas = p + pagesize;
			ctx.avail_meta_area_count = (n-1)*(pagesize>>12);
			ctx.meta_alloc_shift++;
		}
		// printf("alloc_meta 16\n");
		...
		// printf("in malloc, check = %d, secret = %d\n", ctx.meta_area_tail->check, ctx.secret);
...
static struct meta *alloc_group(int sc, size_t req)
{
	...
	int i = 0, cnt;
	unsigned char *p;
	// printf("into alloc group\n");
	struct meta *m = alloc_meta();
	// printf("alloc group, should return from here\n");
	if (!m) return 0;
	// printf("m is not null, should be printed\n");
...
static int alloc_slot(int sc, size_t req)
{
	...
	// printf("alloc slot should return from here\n");
	if (!g) return -1;
	// printf("alloc slot unreachable\n");
