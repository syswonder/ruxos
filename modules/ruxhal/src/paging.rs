...
use crate::mem::{
    memory_regions, phys_to_virt, virt_to_phys, MemRegionFlags, PhysAddr, VirtAddr, PAGE_SIZE_4K,
};
...
static KERNEL_PAGE_TABLE: LazyInit<SpinNoIrq<PageTable>> = LazyInit::new();
...
pub fn do_pte_map(vaddr: VirtAddr, fake_vaddr: VirtAddr, flags: MappingFlags) -> PagingResult {
    KERNEL_PAGE_TABLE
        .lock()
        .map(vaddr, virt_to_phys(fake_vaddr), PageSize::Size4K, flags)
}
...
/// Check if the virtual address has been mapped.
pub fn pte_map_check(vaddr: VirtAddr) -> PagingResult {
    KERNEL_PAGE_TABLE.lock().query(vaddr)?;
    Ok(())
}
